...
int VitaMTP_Get_Vitas (vita_raw_device_t **p_raw_devices) {
    ...
    while ((dev = devs[i++]) != NULL) {
        ...
        if (libusb_get_string_descriptor_ascii (handle, desc.iSerialNumber, (unsigned char *)vitas[n].serial, sizeof (vitas[n].serial)) < 0) {
...
vita_device_t *VitaMTP_Get_First_Vita(void){
    ...
    if ((numdevs = VitaMTP_Get_Vitas (&devices)) < 0){
        return NULL;
    }
    ...
    first_device = VitaMTP_Open_Vita (&devices[0]);
    
    VitaMTP_Unget_Vitas (devices, numdevs);
    return first_device;
}
